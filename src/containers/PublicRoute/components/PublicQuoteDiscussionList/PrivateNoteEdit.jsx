...
class PrivateNoteEdit extends Component {
   fileObj = [];
   fileArray = [];
   constructor(props) {
      super(props);
   ...
   onUpdateNote = () => {
      ...
      axios.put(`/quotes/private-note/${privateNote._id}`, { content: noteContent, files: this.fileArray, entoken })
         .then(({ data }) => {
   ...
   componentDidUpdate(prevProps, prevState) {
      if (prevProps.isEditMode !== this.props.isEditMode) this.setState({
         fileArray: this.props.privateNote.files || [],
         noteContent: this.props.privateNote.content,
      });
   }
   render() {
      const { privateNote, isEditMode } = this.props;
      const { quote } = this.props;
      console.log(" dddddddddddd this.prosp === ", privateNote)
      return (
         <div className={clsx("discussIsEdit", isEditMode ? "" : "isHidden")}>
            <TextareaAutosize
               className="form-control mb-2"
               name="privateNote-content-input"
               rows={4}
               placeholder=""
               value={this.state.noteContent}
               onChange={(ev) => this.setState({ noteContent: ev.target.value })}
            />
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(PrivateNoteEdit))
